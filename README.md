...
## 개발 방법
1. 간단한 CLI 기반 DID 시스템 구현 및 검증
- SSI Korea 포럼에서 제공한 기본 오픈소스를 활용
- CLI 기반 DID 자격증명 발급, 보유, 제시, 검증 클라이언트 구현
- 이더리움 스마트 컨트랙트 기반 VDR(Verifiable Data Registry) 서버 구현
  * 이더리움2 노드 s/w (RPC server 포함)
  * ssl reverse proxy
2. DID 자격증명을 학생증, 성적 증명서로 대체하여 업그레이드 구축 및 검증
3. CLI 기반 시스템을 GUI 기반 시스템으로 업그레이드 구축 및 검증
